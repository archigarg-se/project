...
function isInvalidData(data, rules) {
  ...
  if (data.type === "temperature" && (data.value < -50 || data.value > 500)) return true;
  if (data.type === "humidity" && (data.value < 0 || data.value > 100)) return true;
  return false;
}
...
setInterval(async () => {
  ...
  const deviceId = `device-${Math.ceil(Math.random() * 5)}`;
